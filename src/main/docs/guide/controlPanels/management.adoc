...
If your application already uses Micronaut Security, the control-panel-owned HTTP surface is protected by default
without turning the module into a separate OAuth client. Your existing Micronaut Security authentication mechanism
...
`AUTHENTICATED` is the default control panel fallback rule when Micronaut Security is active: it requires a logged-in
user, but it does not choose an application role for you. To restrict the control panel to administrators, configure
role-based access with Micronaut Security in the host application. Host `intercept-url-map` rules run before the
control-panel fallback rule, so they can require a role such as `ROLE_ADMIN` for the UI and helper controller routes:
...
Applications that include Micronaut Security can explicitly restore anonymous control panel access with
`micronaut.control-panel.security.access=ANONYMOUS`. Only do this in trusted development or test environments.
...
If you set `micronaut.control-panel.security.access=ANONYMOUS`, anonymous control-panel requests continue to see the
...
default authenticated control-panel access when you do not want to expose that information to anonymous users.
