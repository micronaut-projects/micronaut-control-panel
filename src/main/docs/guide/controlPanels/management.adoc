...
turning the module into a separate OAuth client. Set `micronaut.control-panel.security.access=AUTHENTICATED` and let
your existing Micronaut Security authentication mechanism authenticate the browser request.

The following example keeps the control panel opt-in and requires an authenticated user before the UI or helper
controllers can be reached:
...
This secures the control panel routes themselves. Management endpoint security remains under the normal Micronaut
management configuration for your application.

`AUTHENTICATED` is the control panel fallback rule: it requires a logged-in user, but it does not choose an application
role for you. To restrict the control panel to administrators, configure role-based access with Micronaut Security in
the host application. Host `intercept-url-map` rules run before the control-panel fallback rule, so they can require a
role such as `ROLE_ADMIN` for the UI and helper controller routes:
  ...
  security:
    intercept-url-map:
      - pattern: /control-panel/**
        access:
          - ROLE_ADMIN
      - pattern: /cache-control-panel-controller/**
        access:
          - ROLE_ADMIN
      - pattern: /datasource-control-panel-controller/**
        access:
          - ROLE_ADMIN
      - pattern: /object-storage-control-panel-controller/**
        access:
          - ROLE_ADMIN
----
...
health endpoint only displays details to authenticated users. When the control panel itself is protected with
`micronaut.control-panel.security.access=AUTHENTICATED`, the current authenticated request is reused and
...
control panel itself with `micronaut.control-panel.security.access=AUTHENTICATED` when you do not want to expose that
information to anonymous users.
