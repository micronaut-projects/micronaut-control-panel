...
Some management panels require their backing Micronaut modules. For example, the Metrics panel appears only when
Micrometer is present and metrics are enabled:

dependency:io.micronaut.micrometer:micronaut-micrometer-core[scope=runtimeOnly]

MongoDB diagnostics require the MongoDB Control Panel module plus `micronaut-mongo-sync` or
